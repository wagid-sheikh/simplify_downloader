"""Use connected payment components in audit view."""

from __future__ import annotations

from alembic import op

revision = "0113_conn_pay_audit"
down_revision = "0112_conn_pay_missing"
branch_labels = None
depends_on = None
...
payment_row_stats AS (
    SELECT
        ...
        COALESCE(array_agg(ct.order_token ORDER BY ct.order_token) FILTER (WHERE ct.order_token IS NOT NULL), ARRAY[]::text[]) AS normalized_order_tokens,
        COALESCE(string_agg(ct.order_token, '|' ORDER BY ct.order_token) FILTER (WHERE ct.order_token IS NOT NULL), '') AS group_key
    FROM payment_base AS pb
...
SELECT
    ...
    CASE
        ...
        WHEN COALESCE(ot.matched_order_count, 0) < COALESCE(ot.token_count, prs.token_count, 0) THEN 'missing order token'

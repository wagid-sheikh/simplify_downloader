"""Seed pipelines and notifications for MTD same-day fulfillment report."""
...
revision = "0096_seed_mtd_same_day_notif"
down_revision = "0095_drop_skip_uc_pending"
branch_labels = None
depends_on = None

PIPELINE_CODE = "reports.mtd_same_day_fulfillment"
PIPELINE_DESCRIPTION = "Reports Pipeline, MTD Same-Day Fulfillment"
...
TEMPLATE_SUBJECT = "MTD Same-Day Fulfillment Report - {{ report_date }} ({{ overall_status }})"
TEMPLATE_BODY = """
{{ summary_text }}
{% if not summary_text %}
...
def _pipelines_table() -> sa.Table:
    return sa.table("pipelines", sa.column("id"), sa.column("code"), sa.column("description"))
...
def _upsert_pipeline(connection: sa.Connection) -> int:
    ...
    stmt = (
        ...
        .on_conflict_do_update(index_elements=[pipelines.c.code], set_={"description": PIPELINE_DESCRIPTION})
...
def _upsert_profile(connection: sa.Connection, *, pipeline_id: int) -> int:
    ...
    description = "MTD same-day fulfillment report notifications"
    stmt = (
...
def _upsert_template(connection: sa.Connection, *, profile_id: int) -> None:
    ...
    stmt = (
        ...
        .on_conflict_do_update(
            ...
            set_={"subject_template": TEMPLATE_SUBJECT, "body_template": TEMPLATE_BODY, "is_active": True},
        )
    )
...
def downgrade() -> None:
    ...
    pipeline_id = connection.execute(sa.select(pipelines.c.id).where(pipelines.c.code == PIPELINE_CODE)).scalar()
    if not pipeline_id:
        return

    profile_ids = connection.execute(sa.select(profiles.c.id).where(profiles.c.pipeline_id == pipeline_id)).scalars().all()

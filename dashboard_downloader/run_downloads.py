...
async def _extract_dashboard_summary(
    page: Page,
    ...
    *,
    ...
    def _set_metric(key: str, raw_value: str | None, parser) -> None:
        parsed_value = parser(raw_value) if parser else raw_value
        dashboard_data[key] = parsed_value
        found_metrics.add(key)
        if parser and parsed_value is None and raw_value is not None and _clean_number(raw_value):
            _log(
                "warn",
    ...
    try:
        if await title_locator.count() == 0:
            ...
            if lines:
                if len(lines) >= 1:
                    # Attempt to parse dashboard date from the last line or <small> element.
    ...
    except Exception as exc:  # pragma: no cover - defensive against selector issues
        _log(
            "warn",
    ...
    try:
        if await launch_locator.count() > 0:
            ...
            _log("warn", "launch date section missing", extras={"section": "Launch Date"})
    ...
    async def _collect_section_rows(section_title: str) -> List[List[str]]:
        heading = page.locator(f"h3.section-title:has-text(\"{section_title}\")")
        try:
            if await heading.count() == 0:
                _log("warn", "section heading missing", extras={"section": section_title})
                return []
        except Exception as exc:  # pragma: no cover
            _log(
                "warn",
                "error locating section heading",
                extras={"section": section_title, "error": str(exc)},
            )
            return []

        table_locator_candidates = [
            heading.first.locator("xpath=following-sibling::table[1]"),
            heading.first.locator("xpath=parent::*//table[1]"),
            heading.first.locator(
                "xpath=ancestor::*[self::section or contains(concat(' ', normalize-space(@class), ' '), ' card ')]//table[1]"
            ),
        ]

        table_locator = None
        for candidate in table_locator_candidates:
            try:
                if await candidate.count() > 0:
                    table_locator = candidate.first
                    break
            except Exception:
                continue

        if table_locator is None:
            _log("warn", "table not found for section", extras={"section": section_title})
            return []
        ...
        try:
            ...
            for idx in range(count):
                cell_locator = row_locator.nth(idx).locator("th,td")
            ...
            _log(
                "warn",
                "failed to read table rows",
                extras={"section": section_title, "error": str(exc)},
            )
        return rows

    revenue_rows = await _collect_section_rows("Revenue")
    revenue_map = {
        "prevmonthrevenue": ("prev_month_revenue", _to_decimal),
        "previousmonthrevenue": ("prev_month_revenue", _to_decimal),
        "targetrevenue": ("target_revenue", _to_decimal),
        "target": ("target_revenue", _to_decimal),
        "lmtdrevenue": ("lmt_d_revenue", _to_decimal),
        "lmtd": ("lmt_d_revenue", _to_decimal),
        "mtdrevenue": ("mtd_revenue", _to_decimal),
        "mtd": ("mtd_revenue", _to_decimal),
        "ftdrevenue": ("ftd_revenue", _to_decimal),
        "ftd": ("ftd_revenue", _to_decimal),
        "tgtvsach": ("tgt_vs_ach_pct", _to_decimal),
        "tgtvsachpct": ("tgt_vs_ach_pct", _to_decimal),
        "growth": ("growth_pct", _to_decimal),
        "growthpct": ("growth_pct", _to_decimal),
        "extrapolated": ("extrapolated_pct", _to_decimal),
        "extrapolatedpct": ("extrapolated_pct", _to_decimal),
    }
    ...
    for row in revenue_rows:
        ...
        value = row[-1]
        mapping = revenue_map.get(label)
        if not mapping:
            continue
        key, parser = mapping
        _set_metric(key, value, parser)

    pickup_rows = await _collect_section_rows("Pickup")
    for row in pickup_rows:
        if not row:
            continue
        label = _normalize_label(row[0])
        if label in {"pickup", "metric"}:
            continue
        if label in {"new", "pickupnew", "newcustomers"}:
            if len(row) > 1:
                _set_metric("pickup_new_count", row[1], _to_int)
            if len(row) > 2:
                _set_metric("pickup_new_conv_count", row[2], _to_int)
            if len(row) > 3:
                _set_metric("pickup_new_conv_pct", row[3], _to_decimal)
            else:
                _log(
                    "warn",
                    "pickup row missing columns",
                    extras={"row": row},
                )
        elif label in {"existing", "pickupexisting", "existingcustomers"}:
            if len(row) > 1:
                _set_metric("pickup_existing_count", row[1], _to_int)
            if len(row) > 2:
                _set_metric("pickup_existing_conv_count", row[2], _to_int)
            if len(row) > 3:
                _set_metric("pickup_existing_conv_pct", row[3], _to_decimal)
            else:
                _log(
                    "warn",
                    "pickup existing row missing columns",
                    extras={"row": row},
                )
        elif label in {"total", "pickuptotal"}:
            if len(row) > 1:
                _set_metric("pickup_total_count", row[1], _to_int)
            if len(row) > 2:
                _set_metric("pickup_total_conv_count", row[2], _to_int)
            if len(row) > 3:
                _set_metric("pickup_total_conv_pct", row[3], _to_decimal)
            else:
                _log(
                    "warn",
                    "pickup total row missing columns",
                    extras={"row": row},
                )

    delivery_rows = await _collect_section_rows("Delivery")
    delivery_map = {
    ...
    for row in delivery_rows:
        if len(row) < 2:
            continue
        label = _normalize_label(row[0])
        key = delivery_map.get(label)
        if not key:
            continue
        value = row[-1]
        if key.endswith("_pct"):
            _set_metric(key, value, _to_decimal)
        else:
            _set_metric(key, value, _to_int)

    repeat_rows = await _collect_section_rows("Repeat Customers")
    repeat_map = {
    ...
    package_rows = await _collect_section_rows("Package")
    for row in package_rows:
        if len(row) < 2:
            continue
        label = _normalize_label(row[0])
        value = row[-1]
        if label in {"target", "packagetarget"}:
            _set_metric("package_target", value, _to_int)
        elif label in {"new", "packagenew"}:
            _set_metric("package_new", value, _to_int)
        elif label in {"ftd", "packageftd"}:
            _set_metric("package_ftd", value, _to_int)
        elif label in {"achievement", "achievementpct", "packageachievement"}:
            _set_metric("package_achievement_pct", value, _to_decimal)
        elif label in {"overall", "packageoverall"}:
            _set_metric("package_overall", value, _to_int)
        elif label in {"nonpackageover800", "nonpkgover800"}:
            _set_metric("package_non_pkg_over_800", value, _to_int)
        elif label in {"nonpackageover800undelivered", "nonpkgover800undelivered"}:
            _set_metric("package_non_pkg_over_800_undelivered", value, _to_int)
...
async def _switch_to_store_dashboard_and_download(
    ...
    *,
    ...
    if settings.database_url:
        try:
            dashboard_data = await _extract_dashboard_summary(page, store_cfg, logger=logger)
            if dashboard_data:

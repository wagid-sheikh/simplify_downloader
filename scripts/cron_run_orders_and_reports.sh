...
# If daily sales fails but pending succeeds, attempt one final rescue pass.
# This keeps the same chained cron model and improves the chance of receiving
# sales report before the meeting window without introducing fixed cron gaps.
if [[ "${daily_rc}" -ne 0 && "${pending_rc}" -eq 0 ]]; then
  log "Daily sales failed while pending succeeded; attempting post-pending rescue run for daily sales"
  rescue_daily_rc=0
  run_step "Script 2b: daily_sales_report_rescue" "./scripts/run_local_reports_daily_sales.sh" 2 15 || rescue_daily_rc=$?
  if [[ "${rescue_daily_rc}" -eq 0 ]]; then
    daily_rc=0
    log "Daily sales rescue run succeeded"
  else
    log "WARNING: Daily sales rescue run failed with exit_code=${rescue_daily_rc}"
  fi
fi

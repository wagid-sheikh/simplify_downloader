...
get_process_tree_candidates() {
  ps -axo pid=,command= | grep -E 'cron_run_orders_and_reports|orders_sync_run_profiler|run_local_reports_daily_sales|run_local_reports_pending_deliveries' | grep -v grep || true
}
...
cleanup_stale_processes_if_any() {
  ...
  section "Checking for stale matching processes from previous run"

  if [[ -n "${lock_pid}" ]] && pid_is_alive "${lock_pid}"; then
    log "Previous lock PID ${lock_pid} is still alive. Active run exists."
    return 1
  fi

  candidates="$(get_process_tree_candidates)"
  if [[ -z "${candidates}" ]]; then
    log "No matching stale processes found."
    return 0
  fi

  log "Found possible matching processes:"
...
EOF
  ...
  while IFS= read -r pid; do
    ...
    log "Terminating stale candidate PID=${pid}"
    terminate_pid_gracefully "${pid}" || true
  done <<EOF
${pids}
EOF

  remaining="$(get_process_tree_candidates)"
  if [[ -n "${remaining}" ]]; then
    log "WARNING: Some matching processes still remain after cleanup attempt:"
    while IFS= read -r line; do
      [[ -n "${line}" ]] && log "  ${line}"
    done <<EOF
${remaining}
EOF
    ...
    log "All stale matching processes cleared."
  fi
...
maybe_cleanup_stale_lock() {
  ...
  if [[ -n "${existing_pid}" ]] && pid_is_alive "${existing_pid}"; then
    ...
    log "Another run is genuinely active. Exiting."
    exit 1
  fi

  log "Lock owner PID is not alive or missing. Treating lock as stale."

  cleanup_stale_processes_if_any "${existing_pid}" || {
    log "Could not safely clean stale state because an active run appears to exist. Exiting."
    exit 1
  }

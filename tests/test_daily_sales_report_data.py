...
@pytest.mark.asyncio
async def test_fetch_daily_sales_report_lead_performance_summary_normalizes_status_bucket_and_store_code(
    tmp_path, monkeypatch
) -> None:
    db_path = tmp_path / "daily_sales_report_lead_performance_normalized.db"
    database_url = f"sqlite+aiosqlite:///{db_path}"
    _create_tables(database_url)
    ...
    async with session_scope(database_url) as session:
        await session.execute(
            sa.text(
                """
                INSERT INTO cost_center (cost_center, description, target_type, is_active)
                VALUES ('CC-UN', 'Uttam Nagar', 'value', 1)
                """
            )
        )
        await session.execute(
            sa.text(
                """
                INSERT INTO store_master (id, cost_center, store_code, store_name, sync_group)
                VALUES (1, 'CC-UN', 'A668', 'Uttam Nagar', 'TD')
                """
            )
        )
        await session.execute(
            sa.text(
                """
                INSERT INTO crm_leads (store_code, status_bucket, pickup_created_at) VALUES
                    ('a668 ', 'COMPLETED', '2026-04-05 02:00:00+00:00'),
                    (' A668', ' cancelled ', '2026-04-06 02:00:00+00:00'),
                    ('A668', 'Pending', '2026-04-07 02:00:00+00:00')
                """
            )
        )
        await session.commit()
    ...
    report = await fetch_daily_sales_report(database_url=database_url, report_date=report_date)
    summary = next(item for item in report.lead_performance_summary if item["store"] == "A668")

    assert summary["total_leads"] == 3
    assert summary["completed_leads"] == 1
    assert summary["cancelled_leads"] == 1
    assert summary["pending_leads"] == 1

...
def _load_migration_module():
    ...
    module_path = project_root / "alembic" / "versions" / "0079_td_uc_store_body_sections.py"
    spec = importlib.util.spec_from_file_location("v0079_td_uc_store_body_sections", module_path)
...
def test_td_uc_store_body_sections_upgrade_and_downgrade(monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        connection.execute(
            sa.text(
                ...
                INSERT INTO email_templates (id, profile_id, body_template, is_active) VALUES
                  (101, 11, '{{ summary_text }}', 1),
                  (102, 12, '{{ summary_text }}', 1),
    ...
    assert upgraded[101] == migration.NEW_BODY_TEMPLATE
    assert upgraded[102] == migration.NEW_BODY_TEMPLATE
    ...
    with engine.begin() as connection:
        connection.execute(
            sa.text(
                "UPDATE email_templates SET body_template = 'manually changed after upgrade' WHERE id = 101"
            )
        )
    ...
    assert downgraded[102] == migration.PREVIOUS_BODY_TEMPLATE

...
def _load_migration_module():
    ...
    module_path = project_root / "alembic" / "versions" / "0075_store_scope_subj_codes.py"
    spec = importlib.util.spec_from_file_location("v0075_store_scope_subj_codes", module_path)
...
def test_store_scope_subject_codes_upgrade_and_downgrade(monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        connection.execute(
            sa.text(
                ...
                INSERT INTO email_templates (id, profile_id, subject_template, is_active) VALUES
                  (101, 11, 'old td subject', 1),
                  (102, 12, 'old uc subject', 1),
    ...
    assert upgraded[101] == "TD Orders Sync – {{ store_code }}"
    assert upgraded[102] == "UC Orders Sync – {{ store_code }}"
    ...
    assert downgraded[101] == "TD Orders Sync – {{ overall_status }}"
    assert downgraded[102] == "UC Orders Sync – {{ overall_status }}"

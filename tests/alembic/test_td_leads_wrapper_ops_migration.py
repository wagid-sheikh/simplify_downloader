...
def _load_migration_module():
    ...
    module_path = project_root / "alembic" / "versions" / "0121_td_leads_wrapper_ops.py"
    spec = importlib.util.spec_from_file_location(
        "v0121_td_leads_wrapper_ops", module_path
    )
...
def test_seed_td_leads_wrapper_ops_upgrade_and_downgrade(
    ...
    with engine.begin() as connection:
        ...
        pipeline_id = connection.execute(
            sa.text("SELECT id FROM pipelines WHERE code = 'td_leads_wrapper_ops'")
        ).scalar_one()
        ...
        profile_env = connection.execute(
            sa.text(
                "SELECT code, scope, env FROM notification_profiles WHERE id = :profile_id"
            ),
            {"profile_id": profile_id},
        ).one()
        assert profile_env.code == "td_leads_wrapper_ops_run"
    ...
    with engine.begin() as connection:
        ...
        pipeline_count = connection.execute(
            sa.text(
                "SELECT COUNT(*) FROM pipelines WHERE code = 'td_leads_wrapper_ops'"
            )
        ).scalar_one()

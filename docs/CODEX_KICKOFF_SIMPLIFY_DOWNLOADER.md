...
* Provide `docker-compose.yml` with services:
  ...
  * **app**: build from repo; run `python -m app.dashboard_downloader.cli ...` inside container.
* Execution example (from server):

  ```bash
  docker compose run --rm app python -m app.dashboard_downloader.cli run \
...
* **Deploy (`.github/workflows/deploy-prod.yml`)**: on push to `main` or manual → SSH to server → `git pull` → `docker compose pull` → `docker compose build app` → `docker compose up -d db` (wait healthy) → `docker compose run --rm app python -m app.dashboard_downloader.cli db upgrade`.

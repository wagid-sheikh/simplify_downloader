...
- **Evidence:** `TARGET_COMPUTE_TYPE` is read from `system_config`; accepted values are case-insensitive. `sales` resolves to `SALES`, while `collection` and `collections` resolve to `COLLECTIONS`. Missing, blank, or invalid values resolve to `SALES`.
- **Implications:**
  ...
  - Daily Sales report generation refreshes persisted `cost_center_targets` progress fields independently of the rendered target mode: `sales_mtd` is always current-MTD `sum(vw_orders.order_amount)`, `collection_mtd` is always the allocated `payment_collections` amount for current-MTD orders under the collections target-achievement allocation rules, `sales_target_met` is based only on `sales_mtd >= sale_target`, and `collection_target_met` is based only on `collection_mtd >= collection_target`. These fields must remain independently correct whether `TARGET_COMPUTE_TYPE` renders `SALES` or `COLLECTIONS` in the Target subsection.
  - For the `COLLECTIONS` target computation only, `payment_collections.payment_date` and `payment_collections.source_type` are ignored. Achievement is scoped by matched `vw_orders.order_date`, using the same report MTD order-date window as current sales MTD logic. Therefore a payment allocated to a prior-month order is excluded from current-month target achievement even if the payment row was captured in the current month, while a payment allocated to a current-month order can count even if `payment_collections.payment_date` is outside the current month. This is intentionally different from APNF, Short Payment, and payment-proof reconciliation, which still use qualifying proof semantics where only currently supported proof source types qualify.
...
- **Implications:** Operators should use a small bounded dry run for smoke testing (`python -m app crm rebuild-order-line-items --source both --from-date YYYY-MM-DD --to-date YYYY-MM-DD --window-days 7 --dry-run`), then run the first full live rebuild without `--resume` (`python -m app crm rebuild-order-line-items --source both`). If that live rebuild is interrupted after writing live progress rows, recover with `python -m app crm rebuild-order-line-items --source both --resume` (or the `order-line-items-rebuild` alias); operators should not run one command per window. Dry-run window results are log-only and do not write `order_line_items_rebuild_progress`; live resume uses only non-dry-run progress rows, so a successful simulation cannot cause a live rebuild window to be skipped or resumed. The rebuild splits the requested range into CRM-safe source windows capped at 30 days (or a lower source/store limit), accepts date/window aliases (`--from-date`, `--to-date`, `--window-days`), uses `store_master.start_date` when no explicit start date is supplied, defaults an omitted end date to the current pipeline date, resolves omitted window size to CRM-safe source windows capped at 30 days, persists live window progress for interrupted live `--resume`, retries retryable failed windows, and reports missing windows at completion. Explicit dates remain available for small-window smoke tests and dry runs. Only `complete_with_rows` and `complete_empty` outcomes replace rows; incomplete source windows preserve existing data and emit resumable checkpoints. TD Over Due Popup now emits warning final telemetry with resumable `orders_overdue_popup_blocked` skipped-window details; after store staff clear overdue orders, operators should rerun with `--resume` to retry those windows without treating them as fatal missing windows.
...
### DL-020 — Package sales rows are logical payment proof
- **Decision:** `sales` rows with `TRIM(LOWER(payment_mode)) = 'package'` are a separate logical payment-proof source for payment/recovery reconciliation. They are matched by the same normalized `(cost_center, order_number)` identity as order/payment proof and are summed by order.
- **Implications:** Sufficient Package sales proof excludes an order from Actual Payments Not Found and can auto-clear `TO_BE_RECOVERED` to `RECOVERED` with `recovery_category='AUTO_CLEARED_PACKAGE_SALES_PAYMENT'`. Package proof is not inserted into `payment_collections`; `payment_collections` proof keeps precedence when both sources are sufficient.

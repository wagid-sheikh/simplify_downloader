...
1. Entry points & tooling
   ...
   - scripts/run_dashboard_pipeline_single_context.sh (and similar scripts):
       python -m app ... (legacy: `python -m simplify_downloader ...`)
       → python -m app ...
   - .github/workflows/deploy-prod.yml:
       python -m app db upgrade (legacy: `python -m simplify_downloader db upgrade`)
...
5. Runtime documentation (not just history)
   ...
   - README.md:
       poetry run python -m app ... (legacy: `python -m simplify_downloader ...`)
       → poetry run python -m app ...
   - docs/CODEX_KICKOFF_SIMPLIFY_DOWNLOADER.md:
       "run `python -m app` inside container" (legacy: `python -m simplify_downloader`)
       → use `python -m app`
   - docs/reporting.md:

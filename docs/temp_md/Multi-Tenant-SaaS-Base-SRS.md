...
* Target deployment: containerized, blue/green deployments for zero-downtime.
...
* pgvector required.
* FastAPI with strictly versioned APIs and all routes to be Async.
...
2. (M, V1) RLS enforces per-tenant isolation; patterns extend to sub-entities (e.g., organization/site) when modeled.
...
6. (M, V1) Password policy meets industry standards (min length ≥12, complexity, breach checks, lockout, rotation/reuse limits).
7. (M, V1) Mandatory email verification and OTP verification (WhatsApp, SMS, Email) with 30-minute validity; retry/lockout window 30 minutes; rate-limited and audited.
8. (M, V1) Device registry captures device ID, OS, app version, last seen, IP; supports device and session revocation.
9. (M, V1) Sessions have idle and absolute timeouts per industry norms; refresh tokens rotated; risk-based lockout supported.
10. (M, V1) API keys are tenant-scoped, unique per environment, rotated per policy, and revocable; issuance and rotation are audited.
...
13. (M, V1) Impersonation requires explicit approval, strict scoping, and full audit (who/when/why); auto-expiration enforced.

### 3.4 Auditing, Security Events & Compliance

14. (M, V1) Append-only audit logs covering auth, privileged actions, messaging, exports, AI usage, impersonation; include who/what/when/where/reason and correlation IDs.
...
17. (M, V1) GDPR/UK-GDPR compliance: data minimization, purpose limitation, retention policies, legal hold, DSAR workflows with defined SLA (industry-standard 30 days) and export in machine-readable, redacted form.

### 3.5 Mobile & Offline-First

18. (M, V1) Offline data capture mandatory; encrypted local storage using platform-approved encryption; only tenant-authorized datasets cached.
19. (M, V1) Offline mutation queue with idempotency keys; retries with exponential backoff; replay-safe and deduplicated server-side.
...
22. (M, V1) Template-based messaging; platform-wide templates versioned; tenants can clone/override within tenant scope; approvals logged.
23. (M, V1) Rate limits per IP/user/tenant/channel (defaults set to industry norms); retries with backoff; DLQ with retention and replay controls.
24. (M, V1) Global and tenant-level kill switches for messaging; must be auditable and reversible.
...
29. (M, V1) AI continually learns from tenant-specific data (inserts/updates) within tenant boundaries; models/data embeddings segregated per tenant.
...
32. (M, V1) Export formats and maximum sizes set to industry norms; redaction supported for sensitive fields.

### 3.9 Non-Functional Guarantees

33. (M, V1) API timeouts and DB statement timeouts enforced; mandatory rate limiting on all endpoints.
...
37. (M, V1) No secrets in code; secrets injected from GitHub secrets per environment.
38. (M, V1) Separate DEV/STAGING/PROD; no prod secrets in non-prod; promotion requires passing CI/CD but no manual gates.
39. (M, V1) Backup and restore at platform level; tenant-safe restore boundaries; periodic restore verification.
...
42. (M, V1) Baseline is authoritative; product repos cannot weaken auth/verification, bypass audit/AI governance, or alter RLS without explicit CTO/CEO approval.
43. (M, V1) Change management follows industry-standard RFC/CAB practices; deviations require documented approvals.

### 3.12 Product Instantiation Model

44. (M, V1) Products are created by copying tagged Baseline versions and applying policy/configuration only; no architectural forks.
45. (M, V1) A product instantiation guide shall document required configuration steps and guardrails.

### 3.13 Integrations

46. (M, V1) Ticketing integration supported.
47. (M, V1) Analytics integration supported.
48. (S, V2) Payment gateway integration planned; design must preserve existing security and isolation guarantees.

### 3.14 Data Residency & Regions

49. (M, V1) Region-agnostic storage with GDPR/UK-GDPR compliance; future region routing rules must be enforceable by policy without refactor.
...
* **Availability:** Industry-standard SLA targets; design for multi-AZ where applicable.
* **Performance:** API p95 latency targets per industry norms; worker SLAs for queue drain; sync latency monitored.
* **Security:** Defense-in-depth with RLS, RBAC, MFA/OTP, secure hashing, TLS everywhere, secrets management.
* **Reliability & Recoverability:** Backup/restore with defined RPO/RTO; regular restore drills.
...
* **Auditability & Compliance:** Immutable audit logs; audit-on-read; DSAR fulfillment and legal hold.
...
* DLQ management and replay tooling; retention durations defined per channel.

...
- `payment_collections` is the verified payment evidence table. For APNF (`Actual Payments Not Found`), Short Payment, and payment-proof reconciliation, only currently supported proof source types qualify: `source_type = 'google_sheet'` and `source_type = 'legacy_sales'` are equivalent verified evidence. `bank_row_id` is reserved for future bank-reconciliation work and is ignored by current reports.
...
- Define `T = vw_orders.order_date`; the canonical default delivery due date is `default_due_date = T + 3`. If a source row lacks `default_due_date`, report/recovery logic must fall back to this same `T + 3` rule rather than old two-day or status-based assumptions.
...
- Pending Delivery is a delivery order where `sales` has no matching row, no valid actual payment proof exists, `recovery_status = 'NONE'`, and `age_days <= 3`; it includes zero-value orders. The canonical age is computed from `T = vw_orders.order_date`, not from `order_status`, and the only normal Pending Delivery bucket is `0-3 days`. Historical rules that used `order_status == "Pending"`, `<= 30 days`, or `0-5`/`6-15`/`16-30` buckets are superseded.
- To Be Recovered is a delivery order where no matching `sales` row exists, no valid actual payment proof exists, and `age_days >= 4`. The automatic aged transition persists `recovery_status = 'TO_BE_RECOVERED'` with `recovery_category = NULL`. The Daily Sales recovery section buckets these rows as `4-10 days`, `11-30 days`, `31-85 days`, `86-90 days`, and `>90 days`.
- Actual payment proof means: for a positive order, allocated qualifying proof plus the ₹1 tolerance covers `vw_orders.order_amount`; for a zero-value order, a valid same-`cost_center`, whole-token matching zero-amount `payment_collections` row exists.
...
- Zero-value orders are valid delivery orders. Zero-value orders with no `sales` row and no valid zero-amount proof appear in Pending Delivery when `age_days <= 3` or transition/show as To Be Recovered when `age_days >= 4`. Zero-value orders with a `sales` row and no proof are not APNF, not Short Payment, and not money-recovery actions.
...
- Daily Sales report generation also refreshes persisted `cost_center_targets` progress fields independently of the rendered target mode: `sales_mtd` is always the current-MTD `sum(vw_orders.order_amount)`, `collection_mtd` is always the allocated `payment_collections` amount for current-MTD orders under the collections target-achievement allocation rules, `sales_target_met` is based only on `sales_mtd >= sale_target`, and `collection_target_met` is based only on `collection_mtd >= collection_target`. These persisted fields must remain independently correct regardless of whether `TARGET_COMPUTE_TYPE` renders the Target subsection in `SALES` or `COLLECTIONS` mode.
- In `COLLECTIONS` mode, target computation is deliberately different from APNF, Short Payment, and payment-proof reconciliation: it intentionally ignores `payment_collections.payment_date` and `payment_collections.source_type`. Achievement is scoped by the matched `vw_orders.order_date`, using the same report MTD order-date window as current sales MTD logic. A payment allocated to a prior-month order is excluded from current-month target achievement even when the payment row was captured in the current month. A payment allocated to a current-month order can count even when `payment_collections.payment_date` is outside the current month, because the metric is actual collections allocated to orders created in the report month.
...
- For collection-target achievement, `payment_collections.order_number` may contain a single order or grouped orders split on comma or slash. Matching remains exact normalized token matching within the same `cost_center`; unmatched tokens are ignored because their `vw_orders.order_date` cannot be proven in-scope. Duplicate payment rows are summed with no order-number deduplication. Single-order rows contribute their actual `payment_collections.amount`, including overpayments. Grouped rows allocate payment to older matched orders first using `vw_orders.order_date ASC, order_number ASC`; only the allocated amount belonging to orders inside the report MTD order-date window contributes. If all grouped orders are current-month orders, count the grouped amount once. If a group spans prior-month and current-month orders, prior/older orders consume payment first and only the remaining allocation counts for current-month orders. Examples: prior `600` + current `400` with grouped payment `800` contributes `200`; prior `600` + current `400` with grouped payment `1200` contributes `600`; two current-month grouped orders with payment `900` contribute `900`.
...
- `app/customer_retention/notifications.py` uses the same DB notification contract tables for owner summaries: `pipelines`, `notification_profiles`, `email_templates`, and `notification_recipients`. Production enablement is seeded by Alembic revision `0133_cfl_notif_seed`: pipeline code `customer_retention_pipeline`, active run-scoped profile code `owner_summary`, and active `summary` email template. Recipient email addresses are intentionally not seeded in code; operators must configure environment-appropriate active rows in `notification_recipients`. If no active recipient matches the run environment (or `any`), delivery fails safely with `no_recipients` instead of sending to an unintended address.

...
- **Daily Sales target mode:** `TARGET_COMPUTE_TYPE` is read from `system_config`. Missing or invalid values default to `SALES`; accepted values are case-insensitive: `sales` => `SALES`, and `collection` / `collections` => `COLLECTIONS`. In `SALES` mode, the Target subsection title remains `Target`, targets use `cost_center_targets.sale_target`, achievement uses current-MTD `sum(vw_orders.order_amount)`, and `TTD`, `Delta`, and `Reqd/Day` use sales achieved. In `COLLECTIONS` mode, the subsection title is `Target (actual collections)`, targets use `cost_center_targets.collection_target`, achievement uses allocated verified collections for orders created in the report MTD window, and `TTD`, `Delta`, and `Reqd/Day` use collections achieved. Child column headers remain unchanged in both modes: `Target`, `Achieved`, `TTD`, `Delta`, `Reqd/Day`. This target computation is different from APNF/Short Payment proof checks: it ignores `payment_collections.payment_date` completely and also ignores `payment_collections.source_type`, treats `source_type` as audit/provenance data rather than an eligibility filter, does not change the visible Collections FTD/MTD/LMTD columns, sums duplicate payment rows without order-number deduplication, counts single-order rows at actual amount including overpayments, ignores unmatched tokens, and allocates grouped comma/slash-delimited `payment_collections.order_number` rows to exact normalized same-`cost_center` order tokens by `vw_orders.order_date ASC, order_number ASC` before counting only current-MTD order allocations. The current-MTD scope is the same report MTD order-date window used by current sales MTD logic: prior-month orders are excluded even if payment rows were captured in the current month, and current-month orders can count even if `payment_collections.payment_date` is outside the current month. All-current-month grouped rows count the grouped amount once; mixed prior/current grouped rows let prior/older orders consume payment first, so prior `600` + current `400` with payment `800` contributes `200`, prior `600` + current `400` with payment `1200` contributes `600`, and two current-month grouped orders with payment `900` contribute `900`. Daily Sales report generation refreshes persisted `cost_center_targets` progress fields independently of the rendered target mode: `sales_mtd` is always current-MTD `sum(vw_orders.order_amount)`, `collection_mtd` is always the allocated `payment_collections` amount for current-MTD orders under these collections target-achievement allocation rules, `sales_target_met` is based only on `sales_mtd >= sale_target`, and `collection_target_met` is based only on `collection_mtd >= collection_target`; these fields must remain independently correct regardless of which `TARGET_COMPUTE_TYPE` is rendered in the Target subsection.
- **Primary paths:**

...
- **Daily Sales target mode:** Operators can switch only the top summary table's Target subsection with DB `system_config.TARGET_COMPUTE_TYPE`: missing/blank/invalid values use `SALES`; `sales` uses sale targets and current-MTD `vw_orders.order_amount`; `collection`/`collections` uses collection targets and allocated current-MTD order collections from `payment_collections.amount`. The `COLLECTIONS` target-achievement query intentionally ignores `payment_collections.source_type`; do not reuse APNF/Short Payment source-type filtering there because `source_type` is audit/provenance data for this target computation, not an eligibility filter. Notification subjects/bodies/templates and the visible Collections FTD/MTD/LMTD columns do not change.

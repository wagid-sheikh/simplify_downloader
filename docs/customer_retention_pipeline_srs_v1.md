...
- [x] `TESTED` Create `app/customer_retention/caps.py` for active cap resolution from `customer_followup_cap_config`.
- [x] `TESTED` Implement cap selection by `lead_source_type`, `work_section`, `cost_center`, and `run_date`.
- [x] `TESTED` Prefer store-specific cap rows over global rows.
- [x] `TESTED` Select latest effective row only within same specificity.
- [x] `TESTED` Detect ambiguous overlapping active cap rows and return a warning or validation error.
- [x] `TESTED` Create `app/customer_retention/workload.py` for rolling 14-day incomplete workload analytics.
- [x] `TESTED` Ensure 14-day freeze logic excludes carry-forward leads older than 14 days.
- [x] `TESTED` Ensure freeze logic applies only to fresh RETENTION generation.
- [x] `TESTED` Create `app/customer_retention/workbook_generator.py` for two-sheet workbooks: `READ_ME` and `FOLLOWUP_LEADS`.
- [x] `TESTED` Generate one workbook per active `store_master.customer_retention_pipeline = true` store.
- [x] `TESTED` Include due follow-ups and pending carry-forward rows above and beyond caps.
- [x] `TESTED` Include TD leads uncapped.
- [x] `TESTED` Apply RETENTION cap only to fresh RETENTION rows.
- [x] `TESTED` Apply EXTERNAL cap only to eligible fresh EXTERNAL rows if configured.
- [x] `TESTED` Protect system-generated workbook columns.
- [x] `TESTED` Unlock only editable workbook columns from SRS Section 25.
- [x] `TESTED` Add strict dropdown validation for `Target Cost Center` from active `store_master` values.
...
- [x] `SIGNED_OFF` Update Phase 4 tracker status and sign-off notes after workbook tests and owner-accepted re-audit pass.


Audit follow-up tasks before Phase 4 sign-off:

- [x] `TESTED` Hardened `Target Cost Center` Excel validation for production-sized active store lists by closing the 2026-06-12 audit blocker with a hidden/protected lookup range or named-range fallback when inline list validation would exceed Excel formula limits; owner accepted this re-audit closure on 2026-06-13.
- [x] `TESTED` Enforced and documented the SRS Section 25 conditional `Target Cost Center` behavior through ingestion semantics: the value is actionable only for `Customer Response = Shifted Location`, and same-store, free-text/invalid, or blank targets remain row-level warnings with no destination lead; owner accepted this re-audit closure on 2026-06-13.
- [x] `TESTED` Added Phase 4 pipeline-flow logging around store loading, due follow-up selection, pending carry-forward selection, TD generation, RETENTION generation/freeze, EXTERNAL cap decisions, and workbook output counts using the existing `JsonLogger`/`log_event` framework; owner accepted this re-audit closure on 2026-06-13.
- [x] `TESTED` Expanded the Phase 4 acceptance-test matrix with dedicated assertions for active/inactive store dropdown population, large store-list validation fallback, conditional Target Cost Center semantics, controlled editable dropdowns, selection warnings for invalid normalized mobile identities, and generated workbook re-ingestion after protected identity metadata is present; owner accepted this re-audit closure on 2026-06-13.

Phase 4 progress notes:

- 2026-06-13 — Owner/Codex re-audit: Phase 4 is signed off. Large active-store `Target Cost Center` validation fallback is implemented and tested; conditional `Target Cost Center` semantics are documented and enforced in ingestion; Phase 4 selection/workbook logging is implemented and tested; acceptance-test coverage was expanded; and focused Phase 4 tests pass. No schema deviation was required.
- 2026-06-12 — Codex: Implemented and tested active DB-backed cap resolution, TD uncapped contract validation, rolling 14-day workload freeze that excludes older carry-forward rows, active-store workbook selection across due/pending/TD/EXTERNAL/fresh RETENTION categories, two-sheet workbook generation, protected/editable column behavior, strict dropdown validation, and Phase 2 ingestion compatibility for generated rows. No schema deviation was required; Phase 1 schema, Phase 2 ingestion/idempotency/mobile identity behavior, and Phase 3 lifecycle/suppression/recovery contracts were preserved. At that time, human review/sign-off remained pending and Phase 4 was not yet marked `SIGNED_OFF`.

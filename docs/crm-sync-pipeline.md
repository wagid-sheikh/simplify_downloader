...
* Report can run multiple times per day, but default behavior skips when a successful (`ok`/`warning`) run already exists for the same report date.
* Use `--force` to re-generate the report and overwrite the PDF output for the same report date.
...
| 34  | `orders.ingest_remarks`         | `stg_td_orders.ingest_remarks`                | Carry forward ingest remarks text (NULL when no validation issues)           |
...
| `stg_uc_orders.ingest_remarks` | Derived during ingest – aggregated validation warnings (NULL when clean) |
| `stg_uc_orders.id`             | Primary key – generated by DB                                            |

**Phase 6 – UC GST ingest:** populate `stg_uc_orders.ingest_remarks` whenever row-level validation produces warnings so the staging table records ingestion context.
...
| 32  | `orders.order_edited_flag`      | `FALSE`                                         | Default edit flag                                                            |
| 33  | `orders.system_order_status`    | `'Active'`                                      | System-level status                                                          |
| 34  | `orders.ingest_remarks`         | `stg_uc_orders.ingest_remarks`                  | Carry forward ingest remarks text (NULL when no validation issues)           |
...
| 19  | *(Not in Excel)*   | stg_bank   | ingest_remarks | Derived during ingest – aggregated validation warnings (NULL when clean) |

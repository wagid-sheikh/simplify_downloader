...
| `stg_td_orders.ingest_remarks` | Derived during ingest – concatenated validation remarks (NULL when no issues) |
...
| 34  | `orders.ingest_remarks`          | `stg_td_orders.ingest_remarks`                 | Carry forward ingest remark text (NULL when no validation issues)            |
| 35  | `orders.created_by`             | `1`                                           | System user id used for ETL                                                  |
| 36  | `orders.created_at`             | pipeline `run_date`                           | Set to ETL run date                                                          |
| 37  | `orders.updated_by`             | `NULL`                                        | Not set at initial load                                                      |
| 38  | `orders.updated_at`             | `NULL`                                        | Not set at initial load                                                      |
| 39  | `orders.run_id`                 | pipeline `run_id`                             | Traceability of ETL run                                                      |
| 40  | `orders.run_date`               | pipeline `run_date`                           | Same as created_at / ETL execution timestamp                                 |
...
| `stg_uc_orders.ingest_remark` | Derived during ingest – aggregated validation warnings (NULL when clean) |
| `stg_uc_orders.id`          | Primary key – generated by DB                        |

**Phase 6 – UC GST ingest:** populate `stg_uc_orders.ingest_remark` whenever row-level validation produces warnings so the staging table records ingestion context.
...
**Phase 7 – UC orders publish:** carry `ingest_remark` from staging into `orders` alongside the existing field mappings.
...
| 32  | `orders.order_edited_flag`      | `FALSE`                                         | Default edit flag                                                            |
| 33  | `orders.system_order_status`    | `'Active'`                                      | System-level status                                                          |
| 34  | `orders.ingest_remark`          | `stg_uc_orders.ingest_remark`                   | Carry forward ingest remark text (NULL when no validation issues)            |
| 35  | `orders.google_maps_url`        | `NULL`                                          | Not populated from staging table                                             |
| 36  | `orders.latitude`               | `NULL`                                          | Not populated from staging table                                             |
| 37  | `orders.longitude`              | `NULL`                                          | Not populated from staging table                                             |
| 38  | `orders.created_by`             | `1`                                             | System user id used for ETL                                                  |
| 39  | `orders.created_at`             | `stg_uc_orders.run_date`                        | Set to ETL run date                                                          |
| 40  | `orders.updated_by`             | `NULL`                                          | Not set at initial load                                                      |
...
| 19  | *(Not in Excel)*   | stg_bank   | ingest_remark | Derived during ingest – aggregated validation warnings (NULL when clean) |

#### `stg_bank` → `bank`

**Phase 8 – Bank ingest/publish:** capture `ingest_remark` in `stg_bank` alongside the parsed fields, and propagate it verbatim into `bank.ingest_remark` whenever rows are inserted or updated.

The table structure of `stg_bank` and `bank` are almost identical. If stg_bank.row_id does not exist in `bank` perform insert else perform update for `bank.comments`, `bank.cost_center`, `bank.order_number`, `bank.category`, `bank.sub_category`, and `bank.ingest_remark`.
----------------------------------------------------------------------------------------------------------------------------------------

...
| `stg_td_orders.ingest_remark` | Derived during ingest – concatenated validation remarks (NULL when no issues) |
...
| 34  | `orders.ingest_remark`          | `stg_td_orders.ingest_remark`                 | Carry forward ingest remark text (NULL when no validation issues)            |

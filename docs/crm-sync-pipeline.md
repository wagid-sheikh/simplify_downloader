...
* DON’T:
    - Don’t copy-paste working code and modify in-place inside dashboard_downloader.
...
* A pipeline folder for this development assignment already exists (but not developed/code written):
  - app/crm_downloader
  - app/crm_downloader/data        (temporary downloaded Excel files)
...
### td_orders_sync
  ...
  - this pipeline can be invoked independently at any time so maintain strict idempotency
  - Input Date Range:
...
### uc_orders_sync
  ...
  - UC CRM does not provide a separate downloadable Sales report
...
### bank_sync
  ...
  - This pipeline can be invoked independently at any time so maintain idempotency
  - Expected source:
  ...
  - deliver a shell script for manual invocation
    ...
    - for run on server inside docker container: `scripts/run_prod_bank_sync.sh`
      - Shell scripts must:
        ...
        - Once a bank excel file is full ingested then it must be renamed to `*bank_processed_{RUN_ID}.xlsx` and moved to a sub-folder `app/bank_sync/data/ingested` so that future runs do not pick this again
### reports
  ...
  - Each sub-pipeline must:
    ...
    - Write output to:
      - generated PDF stored + logged via documents + existing notification framework
  ...
  - Any sub-pipelines can be invoked independently at any time so maintain strict idempotency
  - deliver a shell script for manual invocation for each sub-pipeline
    ...
    - deliver a shell script to run all the reports in one go
      - for local run: scripts/reports/run_local_all_reports.sh
        ...
        - Detailed logs must capture which ones failed and which succeeded
...
* Must follow & use exisitng infra in-place
  * using & creating record in pipelines table for every pipeline run
    ...
    * seed pipelines table (code='reports', description='Reports Pipeline')
  ...
  * using notification_profiles & create needed profile for development of the modules in this document
    * seed notification_profiles: sample code for illustration
      ```sql
            ...
            insert into notification_profiles
            select id, 'reports', 'Reports Sync', 'any', null, null, true from pipelines where code='reports';
      ```
...
* **store_master** table alter requirements for making this pipeline work smoothly
  ...
  * alter store_master and add following columns, and seed/update data as described below
    ...
    * sync_sales_flag boolean default false
    * sync_bank_flag boolean default false
    * sync_config JSONB default null
    * sync_group char(2) default null
    * Update store_master.cost_center as
      * for store_code 'A668' set cost_center='UN3668'
      * for store_code 'A817' set cost_center='KN3817'
      * for store_code 'UC567' set cost_center='SC3567'
      * for store_code 'UC610' set cost_center='SL1610'
    * Update store_master.start_date as
      * for store_code 'A668' set start_date='01-Mar-2025'
      * for store_code 'UC567' set start_date='08-Feb-2025'
      * for store_code 'A817' set start_date='10-May-2025'
      * for store_code 'UC610' set start_date='11-May-2025'
    * for store_code in (A668, A817, UC567, UC610)
      * set sync_orders_flag = true
      * set sync_bank_flag = true
    * for store_code in (A668, A817)
      * set sync_sales_flag = true
      * set sync_group = 'TD'
    * for store_code in (UC567, UC610)
      * set sync_group = 'UC'

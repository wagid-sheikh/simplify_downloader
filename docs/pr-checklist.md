...
- [ ] For Daily Sales target/config changes, I verified `TARGET_COMPUTE_TYPE` behavior: DB `system_config` lookup, missing/invalid default to `SALES`, case-insensitive accepted values, unchanged Collections FTD/MTD/LMTD columns, and correct grouped-payment allocation for `COLLECTIONS` mode.

...
from app.dashboard_downloader.db_tables import email_templates, notification_profiles, notification_recipients, pipelines
from app.dashboard_downloader.json_logger import JsonLogger, log_event
from app.dashboard_downloader.notifications import EmailPlan, _collect_recipient_lists, _load_smtp_config, _send_email
...
async def send_owner_summary(
    ...
    *,
    ...
    subject = _render(template.get("subject_template") or DEFAULT_SUBJECT_TEMPLATE, payload)
    body = _render(template.get("body_template") or DEFAULT_BODY_TEMPLATE, payload)
    if skip_email:
        _log(logger, "info", "customer_retention_owner_summary_email_skipped", reason="skip_email")
        return NotificationResult(planned=1, sent=0, skipped=True, reason="skip_email", subject=subject, body=body)
...
async def _load_notification_contract(session: AsyncSession, *, env: str | None) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    def _has_notification_tables(sync_session: Any) -> bool:
        inspector = sa.inspect(sync_session.connection())
        required = {"pipelines", "notification_profiles", "email_templates", "notification_recipients"}
        return required.issubset(set(inspector.get_table_names()))

    if not await session.run_sync(_has_notification_tables):
        return {"id": None, "code": PROFILE_CODE}, {"subject_template": DEFAULT_SUBJECT_TEMPLATE, "body_template": DEFAULT_BODY_TEMPLATE}, []
    pipeline_id = (await session.execute(sa.select(pipelines.c.id).where(pipelines.c.code == PIPELINE_CODE))).scalar_one_or_none()
    profile_row = None
    if pipeline_id is not None:
        profile_row = (await session.execute(sa.select(notification_profiles).where(notification_profiles.c.pipeline_id == pipeline_id, notification_profiles.c.scope == "run", notification_profiles.c.is_active.is_(True), sa.or_(notification_profiles.c.env == env, notification_profiles.c.env.is_(None))).order_by(notification_profiles.c.env.desc().nullslast(), notification_profiles.c.id.asc()).limit(1))).mappings().first()
    profile = dict(profile_row or {"id": None, "code": PROFILE_CODE})
    template_row = None
    if profile.get("id") is not None:
        template_row = (await session.execute(sa.select(email_templates).where(email_templates.c.profile_id == profile["id"], email_templates.c.is_active.is_(True)).order_by(email_templates.c.id.asc()).limit(1))).mappings().first()
    template = dict(template_row or {"subject_template": DEFAULT_SUBJECT_TEMPLATE, "body_template": DEFAULT_BODY_TEMPLATE})
    recipients: list[dict[str, Any]] = []
    if profile.get("id") is not None:
        rows = (await session.execute(sa.select(notification_recipients).where(notification_recipients.c.profile_id == profile["id"], notification_recipients.c.is_active.is_(True), sa.or_(notification_recipients.c.env == env, notification_recipients.c.env.is_(None))))).mappings().all()
...
def _log(logger: JsonLogger | None, status: str, message: str, **extras: Any) -> None:
    if logger:
        log_event(logger=logger, phase="email", status=status, message=message, extras=extras)

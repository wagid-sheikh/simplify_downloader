...
async def _import_td_leads(session: AsyncSession, pipeline_run_id: str, logger: JsonLogger | None = None) -> AdapterConversionResult:
...
async def import_td_leads(*, database_url: str, pipeline_run_id: str, logger: JsonLogger | None = None) -> AdapterConversionResult:
    """Standalone wrapper that owns its transaction for TD lead conversion."""

    async with session_scope(database_url) as session:
        result = await _import_td_leads(session, pipeline_run_id, logger=logger)

...
def _compute_text_field_layout(
    page_width: float, left_margin: float, right_margin: float, gap_px: int
):
    weights = [
        0.16,  # Order No
        0.16,  # Order Date
...
def _render_pdf(rows: list[_AssignmentRow], base_dir: Path) -> Path:
    ...
    gap_row1_row2 = 5
    gap_row2_row3 = 3
    spacing_between_leads = 8
    checkbox_size = 12
    text_gap_px = 4

    x_positions, field_widths = _compute_text_field_layout(
        page_width, left_margin, right_margin, text_gap_px
    )
    ...
    for row in rows:
        required_height = (
            line_height  # Row 1 text
            + gap_row1_row2
            + line_height  # Row 2 labels
            + gap_row2_row3
            + field_height  # Row 3 fields
            + spacing_between_leads
        )
        ...
        conv_x = max(left_margin - checkbox_size - 2, 0)
        form.checkbox(
        ...
        canvas.setFont("Helvetica", 9)
        canvas.drawString(conv_x + checkbox_size + 2, field_y, "Conv (Y/N)")

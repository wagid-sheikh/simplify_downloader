...
def _resolve_td_api_artifact_dir() -> Path:
    configured = os.environ.get("TD_API_ARTIFACT_DIR", "").strip()
    if configured:
        return Path(configured).expanduser().resolve()
    return Path(__file__).resolve().parents[3] / "docs" / "td_api" / "artifacts"
...
def _persist_api_excel_artifacts(*, store_code: str, from_date: date, to_date: date, result: TdApiFetchResult) -> dict[str, str]:
    base = _resolve_td_api_artifact_dir()
    paths = {
    ...
    try:
        ...
        if source_mode != "ui":
            try:
                ...
                api_artifact_dir = _resolve_td_api_artifact_dir()

...
def _compare_filename(store_code: str, dataset: str, from_date: date, to_date: date) -> str:
    return f"{store_code}_td_compare_{dataset}_{_window_token(from_date)}_{_window_token(to_date)}.json"
...
def persist_td_compare_artifacts(
    *,
    ...
    normalized_store = str(store_code or "").upper()
    shared_health = dict(endpoint_health_summary or {})

    artifacts = [
        ("orders_compare_mismatches", "orders", orders_compare_metrics),
        ("sales_compare_mismatches", "sales", sales_compare_metrics),
    ]
    for artifact_key, dataset, metrics in artifacts:
        payload = {
            "store_code": normalized_store,
            "dataset": dataset,
            "from_date": from_date.isoformat(),
            "to_date": to_date.isoformat(),
            "compare_metrics": dict(metrics or {}),
            "endpoint_health_summary": shared_health,
        }
        path = download_dir / _compare_filename(normalized_store.lower(), dataset, from_date, to_date)
        try:
            _write_json(path, payload)
            result.artifact_paths[artifact_key] = str(path)
        except Exception as exc:  # pragma: no cover - defensive guard
            warning = f"Failed to persist TD compare artifact '{artifact_key}' at {path}: {exc}"
            result.warnings.append(warning)
            logger.warning(warning)
